...
(function () {
	...
	const CONFIG = {
		CDN_BASE: 'https://cdn.jsdelivr.net/gh/tas33n/CHIKA-DONATION-PORTAL@main',
		API_BASE_URL: '/api'
	};
	...
	const API = {
		fetchUserData: function (uid) {
			return fetch(`${CONFIG.API_BASE_URL}/user/${uid}`)
				.then(response => {
		...
		fetchThreadData: function (tid) {
			return fetch(`${CONFIG.API_BASE_URL}/thread/${tid}`)
				.then(response => {
					if (!response.ok) throw new Error('Failed to fetch user data');
					return response.json();
				})
		...
		fetchThreadRawData: function (tid) {
			return fetch(`${CONFIG.API_BASE_URL}/thread/raw/${tid}`)
				.then(response => {
					if (!response.ok) throw new Error('Failed to fetch user data');
					return response.json();
				})
		...
		fetchMe: function () {
			return fetch(`${CONFIG.API_BASE_URL}/me/1`)
				.then(response => {
					if (!response.ok) throw new Error('Failed to fetch user data');
					return response.json();
				})
	...
	`;
		...
		updateSelectedPackage: function () {
		...
		admins: async function () {
			try {
				const response = await fetch(`${CONFIG.CDN_BASE}/data/admins.json`);
				if (!response.ok) {
		...
		supporters: async function () {
			try {
				const response = await fetch(`${CONFIG.CDN_BASE}/data/supporters.json`);
				if (!response.ok) {
					throw new Error('Failed to fetch admin data');
				}
		...
		commands: async function () {
			...
			try {
				const response = await fetch(`${CONFIG.CDN_BASE}/data/commands.json`);
				if (!response.ok) {
		...
		invitePage: async function name() {
			const fetchInviteGroupData = async (groupId) => {
				...
				const response = await fetch(`${CONFIG.API_BASE_URL}/thread/raw/${groupId}`);
			...
			try {
				Utils.showLoading('Fetching Bot official group data...');
		...
		// Add other rendering functions here
	};
	...
	const App = {
		init: function () {
			...
			this.checkBotStatus();
			setInterval(this.checkBotStatus, 60000);
		},
		...
		setupEventListeners: function () {
			...
			$(document).on('click', '.select-package', function () {
				...
				Renderer.updateSelectedPackage();
			});
				...
				if ('serviceWorker' in navigator) {
					try {
						...
						if (registration.active) {
							...
							Utils.showToast('Cache updated successfully!');
			...
			$.ajax({
				...
				success: function (response) {
					if (response.status === 'success') {
						...
						Utils.showToast('Bot server API is not responding correctly. Please contact admin.');
					}
				},
				error: function () {
					Utils.showToast('Bot server API is dead. Please contact admin.');
				}
			});
		}
	};
